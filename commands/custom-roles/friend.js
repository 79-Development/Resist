...
async function handleFriendCommand(client, message, args) {
  ...
  if (!friendRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<:cross:1132996452218585209> | **Friend Role** not found.`);
  ...
  if (hasFriendRole) {
    ...
    embed.setDescription(`<:tick:1132996565049552947> | Successfully removed <@&${friendRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(friendRole);
    embed.setDescription(`<:tick:1132996565049552947> | Successfully added <@&${friendRole}> to ${mentionedMember}`);
  }
