...
async function handleGirlCommand(client, message, args) {
  ...
  if (!girlRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<:cross:1132996452218585209> | **Girl Role** not found.`);
  ...
  if (hasGirlRole) {
    ...
    embed.setDescription(`<:tick:1132996565049552947> | Successfully removed <@&${girlRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(girlRole);
    embed.setDescription(`<:tick:1132996565049552947> | Successfully added <@&${girlRole}> to ${mentionedMember}`);
  }
