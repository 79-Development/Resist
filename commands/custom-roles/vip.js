...
async function handleVipCommand(client, message, args) {
  ...
  if (!vipRole) {
    ...
    const embed = new MessageEmbed()
      .setColor(client.color)
      .setDescription(`<:cross:1132996452218585209> | **VIP Role** not found.`);
  ...
  if (hasVipRole) {
    ...
    embed.setDescription(`<:tick:1132996565049552947> | Successfully removed <@&${vipRole}> from ${mentionedMember}`);
  } else {
    mentionedMember.roles.add(vipRole);
    embed.setDescription(`<:tick:1132996565049552947> | Successfully added <@&${vipRole}> to ${mentionedMember}`);
  }
